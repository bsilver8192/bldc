 ...
 */
static float solenoid_measure_resistance(float current, int samples, bool stop_after) {
	mc_interface_lock();
  ...
  float motor_set = 0;
	...
	while (fabsf(current_set - current) > 0.001) {
		utils_step_towards((float*)&current_set, current, fabsf(current) / 500.0);
    mc_interface_set_current(current_set);
		chThdSleepMilliseconds(1);
	}
